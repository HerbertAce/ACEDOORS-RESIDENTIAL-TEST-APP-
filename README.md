...
Urban production cut-sheet work in v0.4.3:
- The supplied `Urban SD Production Sheet(3).xlsx` now drives Urban panel count and height, muntins, endcaps, struts, insulation cuts, hardware kit, extra-height kit, cable, track, torsion pole, springs, extra CBB and mohair.
- Urban calculations use the existing production allowance: final DLO +10 mm height / +60 mm width.
- The familiar production layout is retained as a compact on-screen comparison matrix, with door fields down the left and up to ten doors across the page.
- Printing automatically splits that matrix into five-door landscape blocks, so a ten-door job prints as two readable blocks without dropping fields.
- Sectional and Roller doors use separate production matrices so product-specific information remains clear.
- Serial Number, Extra Weight and Extra Detail & Notes are available as factory-controlled inputs for each included door.
- Quote, final-measure, accessory, rebate, override and technical-note information flows into the same matrix.
- Undefined combinations in the supplied workbook are shown as `ENGINEERING REVIEW` rather than inventing a value. This currently includes Urban six-panel spring calculations because the workbook has no hardware weight for six-panel combinations.
- Thermal calculations remain on engineering review until a Thermal production parameter sheet is supplied.

Production and QC corrections retained from v0.4.2:
...
Roller spring selections are sourced from the supplied WIP chart. Urban calculations reproduce the supplied workbook rules; all highlighted engineering-review results must be resolved before manufacture.
