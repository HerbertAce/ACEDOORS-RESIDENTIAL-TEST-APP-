# AceDoors Residential v0.4.8

Flat GitHub Pages review release for testing multiple residential clients, department coordination, named technician allocation and pre-handover balance controls in the same browser.

## Multi-job dashboard

The new **Residential Jobs** dashboard stores each test job separately and shows its job number, customer, site address, current status and last-updated time.

- **New Job** creates the next available `RES-######` number and a clean quote.
- **Open** safely saves the current job before loading another client/site record.
- Search finds jobs by job number, customer or site address.
- **Duplicate** copies the customer and door configuration for similar work, assigns a new job number and clears the site address, acceptance, payment, site measures, Production and handover history.
- **Archive** removes a completed or cancelled job from the normal list without deleting it. **Show archived jobs** allows an authorised user to restore it.
- Active and archived jobs cannot share the same quote/job number.
- Existing v0.4.7 multi-job data and earlier single-job browser drafts migrate automatically into the v0.4.8 job list.
- Sales and Master users can create, duplicate, archive and restore jobs. Other departments can open jobs but remain restricted to their authorised workflow sections.

This review build still uses browser storage. Jobs are available only in that browser profile and can be affected if browser site data is cleared. Shared multi-user access, durable backups and larger document storage require the authenticated backend.
...
| Service Coordination | Client contact, site readiness, proposed dates and named technician allocation | Measure allocation can be completed after the commercial gate; installation planning opens after Production release |
| Technician / Service | Final Site Measure, then allocated installation work | A technician sees only jobs allocated to their exact test user name; Install appears after Production release |
| Production | Release, cut sheets, manufacture, parts and QC | Ready for Install and/or Ready for Dispatch |
| Install | Installation handover | QC complete, remaining balance gate complete and booking confirmed by Service Coordination |
| Dispatch | Dispatch handover | QC complete and remaining balance gate complete |
| Accounts / Invoicing | Remaining balance, final invoice and close-out | 25% plus additions is issued at Production Ready; final close-out follows handover |
| Settings | Internal cost logic | Master only |
...
**Quote → Acceptance → Service Coordination → Site Measure → Production → Install / Dispatch → Invoicing**
...
Workflow tabs use the same wording throughout: green **Complete** or yellow **Missing information / In progress**. Completed form and operational cards collapse automatically once, but can be reopened for review.
...
- After an accessory is included, visible minus/plus controls or direct entry allow quantities up to 99 (for example, six extra remotes).
...
- When Production is Ready, Accounts records the remaining 25% plus approved additions. Non-account installation booking and dispatch are blocked until that invoice is issued; approved account jobs follow their PO/account terms.
- Service Coordination can propose an installation date while Production is underway, but can confirm the technician booking only after QC and the remaining-balance gate.
- Install and Dispatch have their own required handover checklists. Accounts cannot finish final close-out until every required route is complete.

## Master cost settings

Internal Roller labour hours, technician rates and lifting-equipment charges have moved out of Sales into the Master-only **Settings** workspace. The screen also lists the information still needed to extend editable costing to Urban, Thermal, accessories, freight and branch-specific rules: labour assumptions, removal/disposal charges, supplier and tier prices, freight/travel policy, Thermal parameters, margins, rounding and effective dates.
